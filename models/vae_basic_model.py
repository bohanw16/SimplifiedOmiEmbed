...
class VaeBasicModel(BasicModel):
    ...
    def __init__(self, param):
        ...
        if param.omics_mode == 'abc':
            self.loss_names = ['recon_A', 'recon_B', 'recon_C']
        if param.omics_mode == 'ab':
            self.loss_names = ['recon_A', 'recon_B']
        elif param.omics_mode == 'b':
            self.loss_names = ['recon_B']
        elif param.omics_mode == 'a':
            self.loss_names = ['recon_A']
        elif param.omics_mode == 'c':
            self.loss_names = ['recon_C']
        ...
        # define the network
        self.netEmbed = networks.define_VAE(param.omics_dims, param.omics_mode,
                                            param.norm_type, param.leaky_slope,
                                            param.dropout_p, param.latent_space_dim, param.init_type, param.init_gain,
                                            self.gpu_ids)

        # define the reconstruction loss
        self.lossFuncRecon = losses.get_loss_func(param.recon_loss)
    ...
    def forward(self):
        ...
        latent, self.recon_omics = self.netEmbed(self.input_omics)
        # define the latent
    ...
    def cal_losses(self):
        ...
        if self.param.omics_mode == 'a' or self.param.omics_mode == 'abc':
            self.loss_recon_A = self.param.k_reconA * self.lossFuncRecon(self.recon_omics[0], self.input_omics[0])
        else:
            self.loss_recon_A = 0

        # Calculate the reconstruction loss for B
        if self.param.omics_mode == 'b' or self.param.omics_mode == 'abc':
            self.loss_recon_B = self.param.k_reconB * self.lossFuncRecon(self.recon_omics[1], self.input_omics[1])
        else:
            self.loss_recon_B = 0

        # Calculate the reconstruction loss for C
        if self.param.omics_mode == 'c' or self.param.omics_mode == 'abc':
            self.loss_recon_C = self.param.k_reconC * self.lossFuncRecon(self.recon_omics[2], self.input_omics[2])
        else:
            self.loss_recon_C = 0
        ...
        # LOSS EMBED
        self.loss_embed = self.loss_recon
